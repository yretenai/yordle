...
namespace yordle::archive {
    wad_file_v1::wad_file_v1(istream &stream) {
        ...
        if (entry_count > 0) {
            entries = make_shared<Array<wad_entry_v1>>(entry_count, nullptr);
            stream.read(reinterpret_cast<char *>(entries->data()), static_cast<streamsize>(entries->byte_size()));
        }
    }

    shared_ptr<Array<uint8_t>> wad_file_v1::read_file(istream &stream, const wad_entry_v1 &entry) {
    ...
    [[nodiscard]] bool wad_file_v1::has_file(uint64_t hash) const {
        return any_of(entries->begin(), entries->end(), [&](wad_entry_v1 entry) { return entry.hash == hash; });
    }

    [[nodiscard]] bool wad_file_v1::has_file(const filesystem::path &path) const {
