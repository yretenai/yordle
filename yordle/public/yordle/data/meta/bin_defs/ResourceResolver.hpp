...
namespace yordle::data::meta {
    struct YORDLE_EXPORT ResourceResolver : public BaseResourceResolver {
        explicit ResourceResolver(const std::shared_ptr<yordle::data::prop::structure_prop> &prop);

        bool is_type(uint32_t type) override {
            return type == 4013559603u || BaseResourceResolver::is_type(type);
        }
    };
}
#define YORDLE_META_DEF_ResourceResolver
#endif
