...
yordle::data::meta::IsOwnerAliveSpawnConditions::IsOwnerAliveSpawnConditions(const std::shared_ptr<yordle::data::prop::structure_prop> &prop) : yordle::data::meta::IVFXSpawnConditions(prop) {
    if(prop == nullptr) {
        return;
    }
    ...
    if(ptr_mConditions != nullptr) {
        for(const auto &set_mConditions_entry : ptr_mConditions->value) {
            ...
            if(ptr_mConditions_entry != nullptr) {
                mConditions.emplace_back(yordle::data::meta::deserialize<yordle::data::meta::IsOwnerAliveConditionData>(ptr_mConditions_entry, 3575093694u));
            }
        }
    }
}
