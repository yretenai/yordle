/// <auto-generated>
// THIS FILE IS AUTO GENERATED DO NOT EDIT
#include <yordle/data/meta/bin_defs/CharacterToolData.hpp>

#include <yordle/data/meta/bin_class.hpp>
#include <yordle/data/prop/empty_prop.hpp>
#include <yordle/data/prop/inline_structure_prop.hpp>
#include <yordle/data/prop/map_prop.hpp>
#include <yordle/data/prop/optional_prop.hpp>
#include <yordle/data/prop/primitive_array_prop.hpp>
#include <yordle/data/prop/primitive_prop.hpp>
#include <yordle/data/prop/set_prop.hpp>
#include <yordle/data/prop/string_prop.hpp>
#include <yordle/data/prop/unordered_set_prop.hpp>
#include <yordle/data/prop/structure_prop.hpp>

yordle::data::meta::CharacterToolData::CharacterToolData(const std::shared_ptr<yordle::data::prop::structure_prop> &prop) : yordle::data::meta::bin_class(prop) {
    if(prop == nullptr) {
        return;
    }
    auto ptr_searchTags = prop->cast_prop<yordle::data::prop::string_prop>(4179647924u);
    if(ptr_searchTags != nullptr) {
        searchTags = ptr_searchTags->value;
    }

    auto ptr_searchTagsSecondary = prop->cast_prop<yordle::data::prop::string_prop>(2378135198u);
    if(ptr_searchTagsSecondary != nullptr) {
        searchTagsSecondary = ptr_searchTagsSecondary->value;
    }

    auto ptr_championId = prop->cast_prop<yordle::data::prop::int32_prop>(75464251u);
    if(ptr_championId != nullptr) {
        championId = ptr_championId->value;
    }
}
