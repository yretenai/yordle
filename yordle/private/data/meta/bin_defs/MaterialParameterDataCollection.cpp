...
yordle::data::meta::MaterialParameterDataCollection::MaterialParameterDataCollection(const std::shared_ptr<yordle::data::prop::structure_prop> &prop) : yordle::data::meta::bin_class(prop) {
    if(prop == nullptr) {
        return;
    }
    ...
    if(ptr_data != nullptr) {
        for(const auto &set_data_pair : ptr_data->value) {
            ...
            if(ptr_data_key != nullptr && ptr_data_value != nullptr) {
                data.emplace(ptr_data_key->value, yordle::data::meta::deserialize<yordle::data::meta::MaterialParameterData>(ptr_data_value, 1407148951u));
            }
        }
    }
}
