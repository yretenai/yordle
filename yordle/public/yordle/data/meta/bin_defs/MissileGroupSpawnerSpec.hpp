/// <auto-generated>
// THIS FILE IS AUTO GENERATED DO NOT EDIT
#ifndef YORDLE_META_DEF_MissileGroupSpawnerSpec
#pragma once

#include <memory>

#include <yordle/data/meta/bin_defs/SpellObject.hpp>
#include <yordle/data/meta/bin_class.hpp>
#include <yordle/data/meta/bin_ref.hpp>
#include <yordle/data/prop/structure_prop.hpp>
#include <yordle/yordle_export.h>

namespace yordle::data::meta {
#ifndef YORDLE_META_DEF_SpellObject
    struct SpellObject;
#endif
    struct YORDLE_EXPORT MissileGroupSpawnerSpec : public bin_class {
        explicit MissileGroupSpawnerSpec(const std::shared_ptr<yordle::data::prop::structure_prop> &prop);

        bool is_type(uint32_t type) override {
            return type == 1040594301u;
        }

        yordle::data::meta::bin_ref<yordle::data::meta::SpellObject> mChildMissileSpell { 1585338886 };
    };
}
#define YORDLE_META_DEF_MissileGroupSpawnerSpec
#endif
