...
yordle::data::meta::OneTrueMaterialDriver::OneTrueMaterialDriver(const std::shared_ptr<yordle::data::prop::structure_prop> &prop) : yordle::data::meta::x7a851cd8(prop) {
    if(prop == nullptr) {
        return;
    }
    ...
    if(ptr_mDrivers != nullptr) {
        for(const auto &set_mDrivers_entry : ptr_mDrivers->value) {
            ...
            if(ptr_mDrivers_entry != nullptr) {
                mDrivers.emplace_back(yordle::data::meta::deserialize<yordle::data::meta::x7a851cd8>(ptr_mDrivers_entry, 2055544024u));
            }
        }
    }
}
