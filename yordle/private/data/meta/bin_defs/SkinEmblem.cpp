/// <auto-generated>
// THIS FILE IS AUTO GENERATED DO NOT EDIT
#include <yordle/data/meta/bin_defs/SkinEmblem.hpp>

#include <yordle/data/meta/bin_defs/EmblemData.hpp>
#include <yordle/data/meta/bin_class.hpp>
#include <yordle/data/prop/empty_prop.hpp>
#include <yordle/data/prop/inline_structure_prop.hpp>
#include <yordle/data/prop/map_prop.hpp>
#include <yordle/data/prop/optional_prop.hpp>
#include <yordle/data/prop/primitive_array_prop.hpp>
#include <yordle/data/prop/primitive_prop.hpp>
#include <yordle/data/prop/set_prop.hpp>
#include <yordle/data/prop/string_prop.hpp>
#include <yordle/data/prop/unordered_set_prop.hpp>
#include <yordle/data/prop/structure_prop.hpp>

yordle::data::meta::SkinEmblem::SkinEmblem(const std::shared_ptr<yordle::data::prop::structure_prop> &prop) : yordle::data::meta::bin_class(prop) {
    if(prop == nullptr) {
        return;
    }
    auto ptr_mEmblemData = prop->cast_prop<yordle::data::prop::reference_prop>(1576489838u);
    if(ptr_mEmblemData != nullptr) {
        mEmblemData = yordle::data::meta::bin_ref<yordle::data::meta::EmblemData>(1713655491u, ptr_mEmblemData->value);
    }

    auto ptr_mLoadingScreenAnchor = prop->cast_prop<yordle::data::prop::uint32_prop>(1447999873u);
    if(ptr_mLoadingScreenAnchor != nullptr) {
        mLoadingScreenAnchor = ptr_mLoadingScreenAnchor->value;
    }
}
