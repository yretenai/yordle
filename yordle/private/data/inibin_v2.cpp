...
yordle::data::inibin_v2::inibin_v2(dragon::Array<uint8_t> &buffer) {
    ...
    if (flags.uint8x3) {
        ...
        for (auto i = 0; i < count; ++i) {
            properties[keys[i]] = std::array<uint8_t, 3>{{values[i * 3 + 0], values[i * 3 + 1], values[i * 3 + 2]}};
        }
    }
    ...
    if (flags.float32x3) {
        ...
        for (auto i = 0; i < count; ++i) {
            properties[keys[i]] = std::array<float, 3>{{values[i * 3 + 0], values[i * 3 + 1], values[i * 3 + 2]}};
        }
    }
    ...
    if (flags.float8x2) {
        ...
        for (auto i = 0; i < count; ++i) {
            properties[keys[i]] = std::array<float, 2>{{(float) values[i * 2 + 0] / 10.0f, (float) values[i * 2 + 1] / 10.0f}};
        }
    }
    ...
    if (flags.float32x2) {
        ...
        for (auto i = 0; i < count; ++i) {
            properties[keys[i]] = std::array<float, 2>{{values[i * 2 + 0], values[i * 2 + 1]}};
        }
    }
    ...
    if (flags.float8x3) {
        ...
        auto values = buffer.lpcast<uint8_t>(&ptr, count * 3);
        for (auto i = 0; i < count; ++i) {
            properties[keys[i]] = std::array<float, 3>{{(float) values[i * 3 + 0] / 10.0f, (float) values[i * 3 + 1] / 10.0f, (float) values[i * 3 + 2] / 10.0f}};
        }
    }
    ...
    if (flags.float32x4) {
        ...
        for (auto i = 0; i < count; ++i) {
            properties[keys[i]] = std::array<float, 4>{{values[i * 4 + 0], values[i * 4 + 1], values[i * 4 + 2], values[i * 4 + 3]}};
        }
    }
    ...
    if (flags.string) {
        ...
        for (auto i = 0; i < count; ++i) {
            properties[keys[i]] = std::string(reinterpret_cast<const char *>(str_buf.data() + str_offsets[i]));
        }
    }
}
